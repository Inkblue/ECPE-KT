...
tf.app.flags.DEFINE_string('POS_result', 'data/Word_seg.txt', 'Word segmentation and POS results')  ##
tf.app.flags.DEFINE_string('weibo_embedding_dir', 'Embedding/', 'weibo pre_train dir')  ##
...
tf.app.flags.DEFINE_integer('embedding_epoch', 5, 'weibo pre_train best epoch')
...
def build_model(wordPOS_embedding, LIWC_embedding, word_embedding, pos_embedding, x, sen_len, doc_len, is_training, s_weibo):##
##def build_model(word_embedding, pos_embedding, x, sen_len, doc_len, is_training):
    ...
    s_emo = tf.concat([s_emo, s_weibo], 1) ##加NLPCC2013weibo语料
    s_cause = tf.concat([s_cause, s_weibo], 1) ##加NLPCC2013weibo语料
...
def get_batch_data(x, sen_len, doc_len, is_training, y_emotion, y_cause, y_pair, batch_size, s_weibo, test=False):
    for index in batch_index(len(y_cause), batch_size, test):
        feed_list = [x[index], sen_len[index], doc_len[index], is_training, y_emotion[index], y_cause[index], y_pair[index],s_weibo[index]]
...
def run():
    ...
    # Model Code Block
    word_idx_rev, word_id_mapping, word_embedding, pos_embedding = load_bert_embedding(FLAGS.embedding_dim_word, FLAGS.embedding_dim_pos, 'data/clause_keywords.csv') ## bert编码
    # word_idx_rev, word_id_mapping, word_embedding, pos_embedding = load_w2v_KA(FLAGS.embedding_dim_word,FLAGS.embedding_dim_pos, 'data/clause_keywords.csv', FLAGS.w2v_file) ## w2v编码
    ...
    LIWC_embedding = load_LIWC(FLAGS.SC_LIWC_file, 'data/clause_keywords.csv', FLAGS.embedding_dim_LIWC)  ##
    LIWC_embedding = tf.constant(LIWC_embedding, dtype=tf.float32, name='LIWC_embedding')  ##

    wordPOS_embedding = load_POS(FLAGS.POS_result, 'data/clause_keywords.csv')  ##
    wordPOS_embedding = tf.constant(wordPOS_embedding, dtype=tf.float32, name='wordPOS_embedding')  ##

    print('build model...')
    s_weibo = tf.placeholder(tf.float32, [None, FLAGS.max_doc_len, 2*FLAGS.n_hidden])
    ...
    placeholders = [x, sen_len, doc_len, is_training, y_emotion, y_cause, y_pair,s_weibo]

    pred_emo, pred_cause, pred_pair, reg = build_model(wordPOS_embedding, LIWC_embedding, word_embedding, pos_embedding, x, sen_len, doc_len, is_training, s_weibo) ##
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for fold in range(1, 11):
            ...
            if FLAGS.trans_type == 'cross_road':
                ...
                tr_s_weibo = load_train_s_weibo(FLAGS.weibo_embedding_dir, fold, FLAGS.embedding_epoch)  ## 加载训练集的预训练embedding
                te_s_weibo = load_test_s_weibo(FLAGS.weibo_embedding_dir, fold, FLAGS.embedding_epoch)  ## 加载测试集的预训练embedding
                # print('type(tr_s_weibo)',type(tr_s_weibo))
                # print('type(te_s_weibo)', type(te_s_weibo))

            max_f1_emo, max_f1_cause, max_f1_pair = [-1.] * 3
            print('train docs: {}    test docs: {}'.format(len(tr_x), len(te_x)))
            print('train docs s_weibo: {}    test docs s_weibo: {}'.format(tr_s_weibo.shape, te_s_weibo.shape))  ##测试输出s_weibo是否正确
            for i in range(FLAGS.training_iter):
                start_time, step = time.time(), 1
                # train
                for train, _ in get_batch_data(tr_x, tr_sen_len, tr_doc_len, 1., tr_y_emotion, tr_y_cause, tr_y_pair, FLAGS.batch_size, tr_s_weibo):
                    ...
                    step = step + 1
                # test
                test = [te_x, te_sen_len, te_doc_len, 0., te_y_emotion, te_y_cause, te_y_pair, te_s_weibo]
...
def main(_):
    # 将控制台结果输出到文件里
    f = open('main-weibo-result.txt', 'w+', encoding='utf-8')
    sys.stdout = f
