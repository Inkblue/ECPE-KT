...
def load_test_s_weibo(weibo_embedding_dir, fold, epoch):
    weibo_embedding_file = weibo_embedding_dir+'fold{}_epoch{}_embedding.pkl'.format(fold, epoch)
    g= open(weibo_embedding_file, 'rb')
    s_weibo = pickle.load(g)
    return s_weibo

def load_train_s_weibo(weibo_embedding_dir, fold, epoch):
    if fold == 1:
        weibo_embedding_file = weibo_embedding_dir + 'fold{}_epoch{}_embedding.pkl'.format(2, epoch)
        g = open(weibo_embedding_file, 'rb')
        s_weibo = pickle.load(g)
        for i in range(3,11):
            weibo_embedding_file = weibo_embedding_dir + 'fold{}_epoch{}_embedding.pkl'.format(i, epoch)
            g = open(weibo_embedding_file, 'rb')
            s_weibo_fold = pickle.load(g)
            s_weibo = tf.concat([s_weibo, s_weibo_fold], 0)
    else:
        weibo_embedding_file = weibo_embedding_dir + 'fold{}_epoch{}_embedding.pkl'.format(1, epoch)
        g = open(weibo_embedding_file, 'rb')
        s_weibo = pickle.load(g)
        for i in range(2, 11):
            if i!= fold:
                ...
                s_weibo_fold = pickle.load(g)
                s_weibo = tf.concat([s_weibo, s_weibo_fold], 0)
    # print(s_weibo.shape) # 输出 (1750, 75, 200)
    # print(np.array(s_weibo).shape) #输出 ()
    # print('s_weibo.eval().shape',s_weibo.eval().shape)# 输出 (1750, 75, 200)
    return s_weibo.eval() # tensor转为array
